...
Webhook.site URL. Start the watcher before triggering the pipeline. Replace
the placeholder secret references with the references copied from 1Password:
...
The webhook token is used to verify either Buildkite's HMAC signature or its
plain token header. Omitting `BUILDKITE_WEBHOOK_TOKEN` is useful only for a
short-lived demonstration and causes the watcher to print a security warning.
